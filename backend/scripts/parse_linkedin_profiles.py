...
def extract_profile_from_voyager_api(data: dict) -> dict | None:
    ...
    for item in included:
        ...
        if (
            "Skill" in item_type
            or item.get("name")
            and "skill" in str(item.get("$type", "")).lower()
        ):
...
def extract_embedded_json_from_html(html: str) -> list[dict]:
    ...
    # Pattern 2: More robust: find large JSON objects that contain profile data
                ...
                if data.get("@type") == "Person":
                    ...
                    if data.get("jobTitle"):
                        profile_data["headline"] = (
                            data.get("jobTitle")[0]
                            if isinstance(data.get("jobTitle"), list)
                            else data.get("jobTitle")
                        )
...
def extract_target_profile_from_html(html_content: str, target_username: str = None) -> dict | None:
    ...
    if not profile_data["full_name"]:
        ...
        profile_blocks = re.finditer(
            (
                r'"firstName"\s*:\s*"([^"]+)"[^}]{0,200}'
                r'"lastName"\s*:\s*"([^"]+)"[^}]{0,500}'
                r'"publicIdentifier"\s*:\s*"([^"]+)"'
            ),
            html_text,
        )
        ...
        if parsed:
            ...
            if parsed.get("linkedin_username"):
                if not person.get("linkedin_url"):
                    person["linkedin_url"] = (
                        f"https://linkedin.com/in/{parsed['linkedin_username']}"
                    )
                    updated += 1
