    ...
    try:
        ...
        try:
            client.storage.create_bucket(
                STORAGE_BUCKET, options={"public": True}  # Public so profile pics are accessible
            )
...
def upload_photo(client: Client, local_path: Path, storage_path: str) -> str | None:
    ...
    try:
        ...
        client.storage.from_(STORAGE_BUCKET).upload(
            storage_path, file_data, {"content-type": "image/jpeg", "upsert": "true"}
        )
...
def add_membership_and_consent(
    client: Client, event_id: str, user_id: str, role: str = "attendee"
) -> bool:
...
def main():
    ...
    for i, person in enumerate(with_photos, 1):
        ...
        if create_profile(client, user_id, person, photo_url):
            ...
            created_users.append({"user_id": user_id, "person": person})
