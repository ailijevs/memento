...
| IT-06 | FR-4.3 | GET /profiles/{user_id}/compatibility endpoint: self-request blocked, missing profiles return 404, shared background produces correct score and starters | pytest | Sasha | Yes | Yes | [`backend/tests/test_compatibility_endpoint.py`](https://github.com/ailijevs/memento/blob/main/backend/tests/test_compatibility_endpoint.py) |
| UT-14 | FR-4.3, FR-2.1 | Profile helper functions: _title_case_name normalizes LinkedIn names, _parse_graduation_year extracts year from PDL/Exa date strings | pytest | Sasha | Yes | Yes | [`backend/tests/test_profiles_helpers.py`](https://github.com/ailijevs/memento/blob/main/backend/tests/test_profiles_helpers.py) |
...
| MT-13 | FR-4.3, FR-4.1 | Profile directory sorts by compatibility score and filters by name search; consent-gated profiles excluded from all views | Manual | Sasha | No | N/A | [`docs/testing/manual-reports/manual-test-report-013.md`](manual-reports/manual-test-report-013.md) |
...
| UAT-02 | FR-1.1 | End-to-end email/password signup with verification email delivery and confirmation-link activation | Manual | Marty | No | N/A | [`docs/testing/manual-reports/manual-test-report-014-uat.md`](manual-reports/manual-test-report-014-uat.md) |
